	...
	public function submit() {
		...
		if ($product_info) {
			if (isset($this->request->post['quantity'])) {
				$product_quantity = (int)$this->request->post['quantity'];
			} else if (!empty($this->session->data['boc_product_quantity'])) {
			...
			if (isset($this->request->post['option'])) {
				$checked_options = $this->request->post['option'];
			} else if (!empty($this->session->data['boc_product_option'])) {
				$checked_options = $this->session->data['boc_product_option'];
			} else {
				$checked_options = array();
			}
			
			// var_dump($checked_options);
            ...
            if (!$json) {
				...
				if (empty($checked_options)) {
					...
					foreach ($product_options as $product_option_key=>$product_option_value) {
						if (in_array(intval($product_option_value['product_option_id']),array_keys($checked_options))) {
							foreach ($checked_options as $checked_options_key=>$checked_options_value) {
								...
								if($product_option_value['product_option_id'] == $checked_options_key) {
									if ($product_option_value['type'] == 'select' || $product_option_value['type'] == 'radio' || $product_option_value['type'] == 'image') {
										foreach ($product_option_value['product_option_value'] as $option_value_key=>$option_value) {
									// var_dump($product_option_value['product_option_id']);
									// var_dump($option_value['product_option_value_id']);
									// var_dump($checked_options_value);											
											if($option_value['product_option_value_id'] == $checked_options_value) {
												if ($option_value['price_prefix'] == '+') {
													...
													$boc_price = $option_value['price'];
												} else if ($option_value['price_prefix'] == '*') {
													$boc_price = $boc_price * $option_value['price'];
												} else if ($option_value['price_prefix'] == '/') {
													$boc_price = $boc_price / $option_value['price'];
												} else if ($option_value['price_prefix'] == 'u') {
													$boc_price = $boc_price + (( $boc_price * $option_value['price'] ) / 100);
												...
												if ($option_value['subtract'] && (!$option_value['quantity'] || ($option_value['quantity'] < $product_quantity))) {
													$stock = false;
												}
									...
									} elseif ($product_option_value['type'] == 'checkbox' && is_array($checked_options_value)) {
										foreach ($product_option_value['product_option_value'] as $option_value_key=>$option_value) {
									...
									} elseif ($product_option_value['type'] == 'text' || $product_option_value['type'] == 'textarea' || $product_option_value['type'] == 'file' || $product_option_value['type'] == 'date' || $product_option_value['type'] == 'datetime' || $product_option_value['type'] == 'time') {
										$option_data[] = array(
											'product_option_id'       => $product_option_value['product_option_id'],
					...
					$boc_unit_price = $boc_price + $option_price;
					$boc_total = ($boc_price + $option_price) * $product_quantity;
					$boc_unit_tax = $this->tax->getTax(($boc_price + $option_price), $product_info['tax_class_id']);
				}
