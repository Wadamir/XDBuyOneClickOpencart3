<?php
class ModelExtensionModuleBocorder extends Model {
	...
	public function addBocorderHistory($order_id, $order_status_id, $exan, $comment = '', $notify = false, $override = false) {
		...
		if ($order_info) {
			...
			if ($order_info['payment_address_format']) {
				...
				$text .= $language->get('text_order_status') . ' ' . $data['order_status'] . "\n\n";
				...
				if (in_array('order', (array)$this->config->get('config_mail_alert'))) {
					...
					$data['text_instruction'] = $this->language->get('text_comment');
					$data['comment'] = strip_tags($order_info['comment']);
					$data['text_footer'] = $exan;

					$new_subject = html_entity_decode(sprintf($language->get('text_subject'), $order_info['store_name'], $order_info['order_id']), ENT_QUOTES, 'UTF-8') . ' (' . $data['payment_method'] . ')';
